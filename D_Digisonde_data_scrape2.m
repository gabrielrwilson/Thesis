...
%% Open the IMESA data netcdf
    ...
    try
        ...
        for i=1:length(Processing_Status)
            if(strcmp(Processing_Status(i,:),'DDa'))  
                stat_var = 1;
                break;
            end
        end
        ...
        if(~stat_var)
            for j=1:length(Processing_Status)          
                if(strcmp(Processing_Status(j,:),'CNa'))
                    break;
                end
            end
            if(j==50)
    ...
    if(stat_var==1)
        disp([filename, ': D data exists, skipping to next file.']);
    elseif(stat_var==2)
        disp([filename, ': Files have not been processed for by C yet, skipping.']);
    else        
        try           
            ...
            digisonde_window = ncread(ncfilename,'A_digisonde_access_times');
            data_window = ncread(ncfilename,'B_IMESA_data_window_time'); 
            data_time  = ncread(ncfilename,'B_IMESA_window_time'); 
            plate_energy  = ncread(ncfilename,'1_plate_energy'); 

            % The name of the digisonde site
            data_ursi=ncread(ncfilename,'A_digisonde_access_ursi');    
            netcdf.close(ncid);

            % Read the time and LLA
            alt=ncread(ncfilename,'A_access_alt');    
            lat=ncread(ncfilename,'A_access_lat');    
            lon=ncread(ncfilename,'A_access_lon');    
            lla_time=ncread(ncfilename,'A_access_lla_time');       
            
            % Read the noise floor info            
            dispersion=ncread(ncfilename,'C_dispersion');    
            stats=ncread(ncfilename,'C_Window_statics');  
            overflight_sweep_characteritics2=ncread(ncfilename,...
                'C_overflight_sweep_characteritics2');  
            try
                overflight_sweep_characteritics=ncread(ncfilename,...
                    'C_overflight_sweep_characteritics');  
            catch
                overflight_sweep_characteritics = zeros(...
                    size(overflight_sweep_characteritics2));
            end
...
%                 disp(['Overflgiht Rating: ', num2str(overflight_sweep_characteritics(i,6))]);
                if( ~(overflight_sweep_characteritics(i,6)>10) &&...
                        (overflight_sweep_characteritics2(i,6)>10)  )
%                     disp('GIRO_pinged');
                    ...
                    urls(i,1) = {strrep(strcat(Base_url,location_url,...
                        params_url,from_date_url,...
                        to_date_url),' ','')};
    ...
    %                 webread_options = weboptions('Timeout',inf);
            ...
            %         GIRO = webread(url,'Timeout',60);
                    ...
                    if( sum(isnan(GIRO_data_starts))==0 )
                        ...
                        while(digisonde_data_found==1)
                            ...
                            Digisonde_data(i,j,:).digisonde_times =...
                                strrep(GIRO(index:index+18),'T',' ');
                            index = GIRO_data_starts(2,1);
                            Digisonde_data(i,j).digisonde_CS =...
                                str2double(GIRO(index:index+2));
                            index = GIRO_data_starts(3,1);
                            Digisonde_data(i,j).digisonde_foF2 =...
                                str2double(GIRO(index:index+4));
                            index = GIRO_data_starts(4,1);
                            Digisonde_data(i,j).digisonde_foE =...
                                str2double(GIRO(index:index+4));
                            index = GIRO_data_starts(5,1);
                            Digisonde_data(i,j).digisonde_hmE =...
                                str2double(GIRO(index:index+4));
                            index = GIRO_data_starts(6,1);
                            Digisonde_data(i,j).digisonde_hmF2 =...
                                str2double(GIRO(index:index+4));
                            index = GIRO_data_starts(7,1);
                            Digisonde_data(i,j).digisonde_scaleF2 =...
                                str2double(GIRO(index:index+6));
                    ...
                    Digisonde_data(i,1,:).digisonde_times =...
                        '01-01-2014 00:00:00';
                    ...
                    for j=1:digisonde_data_size
                        if( ischar(Digisonde_data(i,j,:).digisonde_times) )
                            digisonde_times(i,j) = datenum(...
                                char(Digisonde_data(i,j,:).digisonde_times));
                            digisonde_CS(i,j) =...
                                Digisonde_data(i,j).digisonde_CS;
                            digisonde_foF2(i,j) =...
                                Digisonde_data(i,j).digisonde_foF2;
                            ...
                            digisonde_hmE(i,j) =...
                                Digisonde_data(i,j).digisonde_hmE;
                            digisonde_scaleF2(i,j) =...
                                Digisonde_data(i,j).digisonde_scaleF2;
                            digisonde_present(i,j) =...
                                Digisonde_data(i,1).digisonde;
                        else
                            digisonde_times(i,j) =...
                                datenum('01-01-2014 00:00:00');
                            digisonde_CS(i,j) = 0;
                ...
                %% Project to the correct altitude
                    ...
                    %% Get time formats for comparisons            
                        ...
                        if( digisonde_present(i,j)==1 )
                            ...
                            if(~isnan(sat_alt))
                                [digisonde_density_m3(i,j),...
                                    digisonde_density_cm3(i,j)]=...
                                    Chapman_Function_Predictor_2(...
                                        digisonde_foF2(i,j),...
                                        digisonde_hmF2(i,j),...
                                        digisonde_scaleF2(i,j),...
                                        sat_alt);
                    ...
                    disp(['IMESA data too low quality for ',...
                        data_ursi(i,1:5)]);
                end
            end
            ...
            digisonde_density_m3 = digisonde_density_m3...
                (:,1:digisonde_data_size);
            digisonde_density_cm3 = digisonde_density_cm3...
                (:,1:digisonde_data_size);

        %% Save to 
            % Save to a netcdf
            copyfile(ncfilename,ncfullfilename);

            try
                nccreate(ncfullfilename,'D_digisonde_times','Dimensions',...
                {'unique_overflights',unique_overflights,...
                'digisonde_data_size',digisonde_data_size});
            catch
            end
            ncwrite(ncfullfilename,'D_digisonde_times',digisonde_times);
            ncwriteatt(ncfullfilename,'D_digisonde_times','description',...
                'The time of each digisonde sweep during overflight window');

            try
                nccreate(ncfullfilename,'D_digisonde_CS','Dimensions',...
                {'unique_overflights',unique_overflights,...
                'digisonde_data_size',digisonde_data_size});
            catch
            end
            ncwrite(ncfullfilename,'D_digisonde_CS',digisonde_CS);
            ncwriteatt(ncfullfilename,'D_digisonde_CS','description',...
                'The CS of each overflight... no idea what CS stood for');

            try
                nccreate(ncfullfilename,'D_digisonde_foF2','Dimensions',...
                {'unique_overflights',unique_overflights,...
                'digisonde_data_size',digisonde_data_size});
            catch
            end
            ncwrite(ncfullfilename,'D_digisonde_foF2',digisonde_foF2);
            ncwriteatt(ncfullfilename,'D_digisonde_foF2','description'...
                ,'The frequency at F2 peak');

            try
                nccreate(ncfullfilename,'D_digisonde_hmF2','Dimensions',...
                {'unique_overflights',unique_overflights,...
                'digisonde_data_size',digisonde_data_size});
            catch
            end
            ncwrite(ncfullfilename,'D_digisonde_hmF2',digisonde_hmF2);
            ncwriteatt(ncfullfilename,'D_digisonde_hmF2','description',...
                'The height of the F2 peak');

            try
                nccreate(ncfullfilename,'D_digisonde_foE','Dimensions',...
                {'unique_overflights',unique_overflights,...
                'digisonde_data_size',digisonde_data_size});
            catch
            end
            ncwrite(ncfullfilename,'D_digisonde_foE',digisonde_foE);
%             ncwriteatt(ncfullfilename,'D_digisonde_foE','description',...
%             'The scale height of the atmosphere');
            
            try
                nccreate(ncfullfilename,'D_digisonde_hmE','Dimensions',...
                {'unique_overflights',unique_overflights,...
                'digisonde_data_size',digisonde_data_size});
            catch
            end
            ncwrite(ncfullfilename,'D_digisonde_hmE',digisonde_hmE);
%             ncwriteatt(ncfullfilename,'D_digisonde_hmE','description',...
%             'The scale height of the atmosphere');

            try
                nccreate(ncfullfilename,'D_digisonde_scaleF2',...
                'Dimensions',{'unique_overflights',unique_overflights,...
                'digisonde_data_size',digisonde_data_size});
            catch
            end
            ncwrite(ncfullfilename,'D_digisonde_scaleF2',digisonde_scaleF2);
            ncwriteatt(ncfullfilename,'D_digisonde_scaleF2',...
                'description','The scale height of the atmosphere');

            try
                nccreate(ncfullfilename,'D_digisonde_present',...
                'Dimensions',{'unique_overflights',unique_overflights,...
                'digisonde_data_size',digisonde_data_size});
            catch
            end
            ncwrite(ncfullfilename,'D_digisonde_present',digisonde_present);
            ncwriteatt(ncfullfilename,'D_digisonde_present',...
                'description',...
                ['Binary flag of whether the digisonde had data over',...
                'the window (1=success)']);

            try
                nccreate(ncfullfilename,'D_digisonde_density_m3',...
                'Dimensions',{'unique_overflights',unique_overflights,...
                'digisonde_data_size',digisonde_data_size});
            catch
            end
            ncwrite(ncfullfilename,'D_digisonde_density_m3',...
                digisonde_density_m3);
            ncwriteatt(ncfullfilename,'D_digisonde_density_m3',...
                'description',...
                'Density in meters^3 at sat altitude over the window.');

            try
                nccreate(ncfullfilename,'D_digisonde_density_cm3',...
                'Dimensions',{'unique_overflights',unique_overflights,...
                'digisonde_data_size',digisonde_data_size});
            catch
            end
            ncwrite(ncfullfilename,'D_digisonde_density_cm3',...
                digisonde_density_cm3);
            ncwriteatt(ncfullfilename,'D_digisonde_density_cm3',...
                'description',...
                'Density in cm^3 at sat altitude over the window');
            ...
            EM = ME.message;
            error_filename = strcat(NC_folder_name,'\D Error Codes\',...
            strrep(filename,'.nc','_D_error.txt'));
