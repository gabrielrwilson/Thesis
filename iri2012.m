...
%     initialcmd = 'curl -d "model=iri_2012';
    initialcmd = 'curl -insecure -d "model=iri2012';
else % Use the input curldir (possibly the default for that input).
    initialcmd = ['"' fullfile(curldir, 'curl"') ' --insecure -d "model=iri2012'];
end
...
start_array = '41    42    43    44';
data_start = strfind(result,start_array)+length(start_array);
...
data_lines = find(newlines>=data_start,sweeplen+1);
data = sscanf(result(newlines(data_lines(1,1)):newlines(data_lines(1,sweeplen+1))), '%f');
...
% result(newlines(164):newlines(167))