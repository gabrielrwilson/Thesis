...
clear i;
parfor active_file = 1:NC_file_number
    
%% Open the netcdf
    filename=NC_file_list(active_file).name;
    ncfilename=strcat(NC_folder_name,'\',filename);
    netcdf.open(ncfilename);
    data_day = filename(9:17);    
    ...
    try
        ...
        for i=1:length(Processing_Status)
            if(strcmp(Processing_Status(i,:),'EIa'))  
                stat_var = 1;
                break;
            end
        end
        ...
        if(~stat_var)
            for j=1:length(Processing_Status)          
                if(strcmp(Processing_Status(j,:),'DDa'))
                    break;
                end
            end
            if(j==50)
    ...
    if(stat_var==1)
        disp([filename, ': E data exists, skipping to next file.']);
    elseif(stat_var==2)
        disp([filename, ': Files have not been processed for by D yet, skipping.']);
    else        
        try
        i=0;
        % Pull out data from the nc file
        access_time = ncread(ncfilename,'A_digisonde_access_times');

        IMESA_window_time = ncread(ncfilename,'B_IMESA_window_time');
        IMESA_time = ncread(ncfilename,'B_IMESA_data_window_time');
        IMESA_ion_density = ncread(ncfilename,'B_IMESA_window_density');
        IMESA_lla_time = ncread(ncfilename,'B_IMESA_window_lla_time');
        IMESA_lat = ncread(ncfilename,'B_IMESA_window_lat');
        IMESA_lon = ncread(ncfilename,'B_IMESA_window_lon');
        IMESA_alt = ncread(ncfilename,'B_IMESA_window_alt');
        IMESA_rr = ncread(ncfilename,'B_IMESA_window_rsquare'); 

        IMESA_sweep_dispersion =ncread(ncfilename,'C_dispersion');             
        IMESA_data_quality2 =ncread(ncfilename,'C_overflight_sweep_characteritics2');  
        try
            IMESA_data_quality =ncread(ncfilename,'C_overflight_sweep_characteritics');  
        catch
            IMESA_data_quality = zeros(size(IMESA_data_quality2));
        end

        digisonde_present = ncread(ncfilename,'D_digisonde_present');
        digisonde_time = ncread(ncfilename,'D_digisonde_times');
        digisonde_foF2 = ncread(ncfilename,'D_digisonde_foF2');
        digisonde_hmF2 = ncread(ncfilename,'D_digisonde_hmF2');
        digisonde_hmE = ncread(ncfilename,'D_digisonde_hmE');
        digisonde_foE = ncread(ncfilename,'D_digisonde_foE');
        ...
        for i=1:num_overflights
            ...
            if(~(IMESA_data_quality(i,6)>20)&&(IMESA_data_quality(i,6)>10))
...
% short_data_window = nan(num_overflights,2);
        ...
        try
            nccreate(ncfilename,'F_IMESA_short_time','Dimensions',{'num_overflights',num_overflights,'IMESA_short_points',IMESA_short_points});
        catch
        end
        ncwrite(ncfilename,'F_IMESA_short_time',IMESA_short_time);

        try
            nccreate(ncfilename,'F_IMESA_short_density','Dimensions',{'num_overflights',num_overflights,'IMESA_short_points',IMESA_short_points});
        catch
        end
        ncwrite(ncfilename,'F_IMESA_short_density',IMESA_short_density);

        try
            nccreate(ncfilename,'F_IMESA_short_dispersion','Dimensions',{'num_overflights',num_overflights,'IMESA_short_points',IMESA_short_points});
        catch
        end
        ncwrite(ncfilename,'F_IMESA_short_dispersion',IMESA_short_dispersion);

        try
            nccreate(ncfilename,'F_IMESA_short_rr','Dimensions',{'num_overflights',num_overflights,'IMESA_short_points',IMESA_short_points});
        catch
        end
        ncwrite(ncfilename,'F_IMESA_short_rr',IMESA_short_rr);

        try
            nccreate(ncfilename,'IMESA_short_lla_time_rr','Dimensions',{'num_overflights',num_overflights,'IMESA_short_lla_points',IMESA_short_lla_points});
        catch
        end
        ncwrite(ncfilename,'IMESA_short_lla_time_rr',IMESA_short_lla_time);

        try
            nccreate(ncfilename,'F_IMESA_short_lat','Dimensions',{'num_overflights',num_overflights,'IMESA_short_lla_points',IMESA_short_lla_points});
        catch
        end
        ncwrite(ncfilename,'F_IMESA_short_lat',IMESA_short_lat);

        try
            nccreate(ncfilename,'F_IMESA_short_lon','Dimensions',{'num_overflights',num_overflights,'IMESA_short_lla_points',IMESA_short_lla_points});
        catch
        end
        ncwrite(ncfilename,'F_IMESA_short_lon',IMESA_short_lon);

        try
            nccreate(ncfilename,'F_IMESA_short_alt','Dimensions',{'num_overflights',num_overflights,'IMESA_short_lla_points',IMESA_short_lla_points});
        catch
        end
        ncwrite(ncfilename,'F_IMESA_short_alt',IMESA_short_alt);

        try
            nccreate(ncfilename,'F_digisonde_short_indexes','Dimensions',{'num_overflights',num_overflights,'IMESA_short_lla_points',IMESA_short_lla_points});
        catch
        end
        ncwrite(ncfilename,'F_digisonde_short_indexes',digisonde_short_indexes);                

        try
            nccreate(ncfilename,'F_digisonde_short_time','Dimensions',{'num_overflights',num_overflights,'IMESA_short_lla_points',IMESA_short_lla_points});
        catch
        end
        ncwrite(ncfilename,'F_digisonde_short_time',digisonde_short_time);               

        try
            nccreate(ncfilename,'F_digisonde_short_foF2','Dimensions',{'num_overflights',num_overflights,'IMESA_short_lla_points',IMESA_short_lla_points});
        catch
        end
        ncwrite(ncfilename,'F_digisonde_short_foF2',digisonde_short_foF2);                

        try
            nccreate(ncfilename,'F_digisonde_short_hmF2','Dimensions',{'num_overflights',num_overflights,'IMESA_short_lla_points',IMESA_short_lla_points});
        catch
        end
        ncwrite(ncfilename,'F_digisonde_short_hmF2',digisonde_short_hmF2);                

        try
            nccreate(ncfilename,'F_digisonde_short_hmE','Dimensions',{'num_overflights',num_overflights,'IMESA_short_lla_points',IMESA_short_lla_points});
        catch
        end
        ncwrite(ncfilename,'F_digisonde_short_hmE',digisonde_short_hmE);                

        try
            nccreate(ncfilename,'F_digisonde_short_foE','Dimensions',{'num_overflights',num_overflights,'IMESA_short_lla_points',IMESA_short_lla_points});
        catch
        end
        ncwrite(ncfilename,'F_digisonde_short_foE',digisonde_short_foE); 

        try
            nccreate(ncfilename,'F_iri_output','Dimensions',{'num_overflights',num_overflights,'IMESA_short_lla_points',IMESA_short_lla_points,'44',44});
        catch
        end
        ncwrite(ncfilename,'F_iri_output',iri_output);

        try
            nccreate(ncfilename,'F_iri_time','Dimensions',{'num_overflights',num_overflights,'IMESA_short_lla_points',IMESA_short_lla_points});
        catch
        end
        ncwrite(ncfilename,'F_iri_time',iri_time);

        try
            nccreate(ncfilename,'F_iri_electronc_density','Dimensions',{'num_overflights',num_overflights,'IMESA_short_lla_points',IMESA_short_lla_points});
        catch
        end
        ncwrite(ncfilename,'F_iri_electronc_density',iri_electron_density);

        try
            nccreate(ncfilename,'F_iri_O_percentage','Dimensions',{'num_overflights',num_overflights,'IMESA_short_lla_points',IMESA_short_lla_points});
        catch
        end
        ncwrite(ncfilename,'F_iri_O_percentage',iri_O_percentage);

        try
            nccreate(ncfilename,'F_iri_Ion_temperature','Dimensions',{'num_overflights',num_overflights,'IMESA_short_lla_points',IMESA_short_lla_points});
        catch
        end
        ncwrite(ncfilename,'F_iri_Ion_temperature',iri_Ion_temperature);
        ...
        EM = ME.message;
        error_filename = strcat(NC_folder_name,'\E Error Codes\',...
        strrep(filename,'.nc','_E_error.txt'));
...
%         NC_error(6,1) = {['Error Name: ',EM_name]};   
        fprintf(2,['Broke on active_file: ',num2str(active_file),...
            ', Filename: ',filename,' Sweepnumber: ',num2str(i),'\r']);
