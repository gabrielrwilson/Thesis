...
%     Updated_rsquared = zeros(num_sweeps,1);
    ...
    %% Update Netcdf
    try 
        nccreate(filename,'ADC_output','Dimensions',{'num_points',num_sweeps,'MAX_NUM_SWEEP_STEPS',MAX_NUM_SWEEP_STEPS});
    catch
    end
    ncwrite(ncfullfilename,'ADC_output',ADC_output);

    try 
        nccreate(filename,'ADC_value','Dimensions',{'num_points',num_sweeps,'MAX_NUM_SWEEP_STEPS',MAX_NUM_SWEEP_STEPS});
    catch
    end
    ncwrite(ncfullfilename,'ADC_value',ADC_value);

    try 
        nccreate(filename,'ADC_voltage','Dimensions',{'num_points',num_sweeps,'MAX_NUM_SWEEP_STEPS',MAX_NUM_SWEEP_STEPS});
    catch
    end
    ncwrite(ncfullfilename,'ADC_voltage',ADC_voltage);

    try 
        nccreate(filename,'TIA_output_voltage','Dimensions',{'num_points',num_sweeps,'MAX_NUM_SWEEP_STEPS',MAX_NUM_SWEEP_STEPS});
    catch
    end
    ncwrite(ncfullfilename,'TIA_output_voltage',TIA_output_voltage);

    try 
        nccreate(filename,'Anode_current','Dimensions',{'num_points',num_sweeps,'MAX_NUM_SWEEP_STEPS',MAX_NUM_SWEEP_STEPS});
    catch
    end
    ncwrite(ncfullfilename,'Anode_current',Anode_current);

    try 
        nccreate(filename,'Anode_ion_number','Dimensions',{'num_points',num_sweeps,'MAX_NUM_SWEEP_STEPS',MAX_NUM_SWEEP_STEPS});
    catch
    end
    ncwrite(ncfullfilename,'Anode_ion_number',Anode_ion_number);

    try 
        nccreate(filename,'Aperature_ion_number','Dimensions',{'num_points',num_sweeps,'MAX_NUM_SWEEP_STEPS',MAX_NUM_SWEEP_STEPS});
    catch
    end
    ncwrite(ncfullfilename,'Aperature_ion_number',Aperature_ion_number);

    try 
        nccreate(filename,'Incident_ion_density','Dimensions',{'num_points',num_sweeps,'MAX_NUM_SWEEP_STEPS',MAX_NUM_SWEEP_STEPS});
    catch
    end
    ...
    %Fit values
    try 
        nccreate(filename,'Updated_rsquared','Dimensions',{'1',1,'num_points',num_sweeps});
    catch
    end
    ncwrite(ncfullfilename,'Updated_rsquared',Updated_rsquared);

    try 
        nccreate(filename,'Updated_fit_parameters','Dimensions','Dimensions',{'5',5,'4',4,'num_points',num_sweeps});
    catch
    end
    ...
    %Derived from fits
    try 
        nccreate(filename,'Updated_ion_density','Dimensions','Dimensions',{'1',1,'num_points',num_sweeps});
    catch
    end
    ncwrite(ncfullfilename,'Updated_ion_density',Updated_ion_density);

    try 
        nccreate(filename,'Updated_temperature','Dimensions','Dimensions',{'1',1,'num_points',num_sweeps});
    catch
    end
    ncwrite(ncfullfilename,'Updated_temperature',Updated_temperature);

    try 
        nccreate(filename,'Updated_charging','Dimensions','Dimensions',{'1',1,'num_points',num_sweeps});
    catch
    end
    ncwrite(ncfullfilename,'Updated_charging',Updated_charging);

    try 
        nccreate(filename,'Ion_riemann_density','Dimensions','Dimensions',{'1',1,'num_points',num_sweeps});
    catch
    end
